...
let translate (program : sstmt list) : Llvm.llmodule = 
  ...
  let rec build_expr (globalvars : tbl_typ) (localvars : tbl_typ) builder ((_, e) : sexpr) = match e with
    ...
    | SAfunc(rt, bl, s) ->
      ...
      (* build stmt *)
      let rbuilder = build_stmt globals locals fbuilder the_function s in
  ...
  (* Build the code for the given statement; return the builder for
    ...
    and build_stmt (globalvars : tbl_typ) (localvars : tbl_typ) builder the_function =
    function
    ...
    | SBAstmt(b, e) -> 
      (* bind and assign *)
      let b = build_stmt globalvars localvars builder the_function (SBstmt(b)) in
      ignore(build_expr globalvars localvars b (A.Void, e)); builder
    | SBlock(sl) -> build_stmt_list globalvars localvars builder the_function sl
    ...
    | SIf (predicate, then_stmt, else_stmt) ->
      ...
      add_terminal (build_stmt globalvars localvars (L.builder_at_end context then_bb) the_function then_stmt) build_br_end;

      let else_bb = L.append_block context "else" the_function in
      add_terminal (build_stmt globalvars localvars (L.builder_at_end context else_bb) the_function else_stmt) build_br_end;
      
      ignore(L.build_cond_br bool_val then_bb else_bb builder);
      L.builder_at_end context end_bb

    | SFor (e1, e2, e3, body) -> 
      build_stmt globalvars localvars builder the_function ( SBlock [e1 ; SWhile (e2, SBlock [body ; SExpr e3]) ] )
    ...
    | SWhile (predicate, body) ->
      ...
      add_terminal (build_stmt globalvars localvars (L.builder_at_end context body_bb) the_function body) build_br_while;
  ...
  and build_stmt_list (globalvars : tbl_typ) (localvars : tbl_typ) builder the_function = function
  | [] -> builder
  | s :: sl -> 
    let b = build_stmt globalvars localvars builder the_function s in
    build_stmt_list globalvars localvars b the_function sl
  in
  ...
  let ret = build_stmt_list globals locals builder main program in
